...
do_cleanup() {
  ...
  if [ "$CLEAN_AFTER_BUILD" = true ]; then
    ...
    if [ "$BUILD_OVERLAY" = true ]; then
      echo ">> [$CURRENT_DATE] Removing $TMP_DIR for device $codename" | tee -a "$DEBUG_LOG"
      cd "$TMP_DIR"
      rm -rf ./* || true
    else
      cd "$source_dir"
      echo ">> [$CURRENT_DATE] Removing $PWD/out" | tee -a "$DEBUG_LOG"
      rm -rf out || true
      echo ">> [$CURRENT_DATE] Removing $PWD/vendor" | tee -a "$DEBUG_LOG"
      rm -rf vendor/* || true
      echo ">> [$CURRENT_DATE] Removing $PWD/.repo/local_manifests/roomservice.xml" | tee -a "$DEBUG_LOG"
  ...
  if [ "$INIT_MIRROR" = true ]; then
    if [ ! -d .repo ]; then
      ...
      if [ "$USE_CN_MIRROR" = true ]; then
        ( yes||: ) | repo init -u https://mirrors.bfsu.edu.cn/git/lineageOS/LineageOS/mirror --mirror --no-clone-bundle -p linux --git-lfs &>> "$repo_log"
      else
...
for codename in ${devices//,/ }; do
  if [ -n "$codename" ]; then
    ...
    if [ "$INCLUDE_PROPRIETARY" = true ]; then
      if [ "$USE_CN_MIRROR" = true ]; then
        wget -q -O .repo/local_manifests/proprietary.xml "https://ghfast.top/https://raw.githubusercontent.com/TheMuppets/manifests/$themuppets_branch/muppets.xml"
    ...
    if [ "$CALL_REPO_INIT" = true ]; then
      ...
      if [ "$LOCAL_MIRROR" = true ]; then
        if [ "$USE_CN_MIRROR" = true ]; then
          ( yes||: ) | repo init -u https://mirrors.bfsu.edu.cn/git/lineageOS/LineageOS/android.git --reference "$MIRROR_DIR" -b "$branch" -g default,-darwin,-muppets,muppets_"${codename}" --git-lfs &>> "$repo_log"
        else
        ...
        if [ "$USE_CN_MIRROR" = true ]; then
          ( yes||: ) | repo init -u https://mirrors.bfsu.edu.cn/git/lineageOS/LineageOS/android.git -b "$branch" -g default,-darwin,-muppets,muppets_"${codename}" --git-lfs &>> "$repo_log"
        else
    ...
    if [ "$USE_CN_MIRROR" = true ]; then
      xmlstarlet ed -L \
        -u "/manifest/remote[@name='github']/@fetch" -v "https://github.com/" \
        -d "/manifest/remote[@name='github']/@review" \
        -a "/manifest/remote[3]" -t elem -n "remote" \
        -i "/manifest/remote[4]" -t attr -n "name" -v "lineage" \
        -i "/manifest/remote[4]" -t attr -n "fetch" -v "https://mirrors.tuna.tsinghua.edu.cn/git/lineageOS/" \
        -i "/manifest/remote[4]" -t attr -n "review" -v "review.lineageos.org" \
        -u "/manifest/default/@remote" -v "lineage" .repo/manifests/default.xml
    fi
    ...
    if [ $repo_sync_returncode -ne 0 ]; then
        echo ">> [$CURRENT_DATE] repo sync failed for $codename, $branch branch" | tee -a "$DEBUG_LOG"
    ...
    if [ "$WITH_GMS" = true ]; then
      echo ">> [$CURRENT_DATE] Populating vendor/partner_gms-tv/products" | tee -a "$repo_log"
...
if [ -f /root/userscripts/end.sh ]; then
  ...
  /root/userscripts/end.sh || echo ">> [$CURRENT_DATE] Error: end.sh failed!"
fi
