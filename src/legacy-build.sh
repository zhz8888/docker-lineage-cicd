...
if [ "$LOCAL_MIRROR" = true ]; then
  ...
  if [ "$INCLUDE_PROPRIETARY" = true ]; then
    if [ "$USE_CN_MIRROR" = true ]; then
      wget -q -O .repo/local_manifests/proprietary.xml "https://ghfast.top/https://raw.githubusercontent.com/TheMuppets/manifests/mirror/default.xml"
      ...
      /root/build_manifest.py --remote "https://github.com" --remotename "github_https" \
        "https://github.com/TheMuppets/manifests/raw/mirror/default.xml" .repo/local_manifests/proprietary_github.xml
    fi
  fi
...
for branch in ${BRANCH_NAME//,/ }; do
  ...
  if [ -n "$branch" ] && [ -n "$devices" ]; then
    ...
    if [ "$CALL_REPO_INIT" = true ]; then
      ...
      if [ "$LOCAL_MIRROR" = true ]; then
        if [ "$USE_CN_MIRROR" = true ]; then
          ( yes||: ) | repo init -u https://mirrors.bfsu.edu.cn/git/lineageOS/LineageOS/android.git --reference "$MIRROR_DIR" -b "$branch" --git-lfs &>> "$repo_log"
        else
          ( yes||: ) | repo init -u https://github.com/LineageOS/android.git --reference "$MIRROR_DIR" -b "$branch" --git-lfs &>> "$repo_log"
        fi
      else
        if [ "$USE_CN_MIRROR" = true ]; then
          ( yes||: ) | repo init -u https://mirrors.bfsu.edu.cn/git/lineageOS/LineageOS/android.git -b "$branch" --git-lfs &>> "$repo_log"
        else
